...
class FeedbackActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        _activityNoteAddUpdateBinding = ActivityFeedbackBinding.inflate(layoutInflater)
        setContentView(binding?.root)

        noteAddUpdateViewModel = obtainViewModel(this@FeedbackActivity)

        feedback = intent.getParcelableExtra(EXTRA_NOTE)
        if (feedback != null) {
            isEdit = true
        } else {
            feedback = Feedback()
        ...
        if (isEdit) {
            ...
            if (feedback != null) {
                feedback?.let { note ->
                    binding?.edtTitle?.setText(feedback!!.name)
                    binding?.edtDescription?.setText(feedback!!.feedback)
                }
            }
        } else {
        ...
        binding?.btnSubmit?.setOnClickListener {
            ...
            when {
                ...
                description.isEmpty() -> {
                    ...
                    if (isEdit) {
                        noteAddUpdateViewModel.update(feedback as Feedback)
                        showToast(getString(R.string.changed))
                    } else {
                        feedback.let { feedback ->
                            feedback?.date = DateHelper.getCurrentDate()
                        }
                        noteAddUpdateViewModel.insert(feedback as Feedback)
    ...
    private fun obtainViewModel(activity: AppCompatActivity): FeedbackViewModel {
        val factory = ViewModelFactorys.getInstance(activity.application)
        return ViewModelProvider(activity, factory).get(FeedbackViewModel::class.java)
    }
    ...
    private fun showAlertDialog(type: Int) {
        ...
        with(alertDialogBuilder) {
            ...
            setPositiveButton(getString(R.string.yes)) { _, _ ->
                if (!isDialogClose) {
                    noteAddUpdateViewModel.delete(feedback as Feedback)
